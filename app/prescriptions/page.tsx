...
import { Pill, Download, RefreshCw, Clock, CheckCircle2, AlertCircle, Search, Loader2 } from "lucide-react";
...
const STATUS_MAP: Record<string, { label: string; badge: string; icon: React.ElementType }> = {
  ...
  ON_HOLD:   { label: "On Hold",   badge: "badge-warning", icon: AlertCircle  },
};
...
export default function PrescriptionsPage() {
    ...
    async function load() {
      try {
        ...
        if (!res.ok) {
          ...
          console.error("[PrescriptionsPage] fetch failed:", res.status, body);
          return;
        }
        const { prescriptions: data } = await res.json();
        setPrescriptions(data || []);
      } catch (err) {
        console.error("[PrescriptionsPage] load failed:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const filtered = prescriptions.filter((rx) =>
    ...
    (search === "" ||
      ...
      rx.medications.some((m) => m.medicationName.toLowerCase().includes(search.toLowerCase())))
  );
    ...
    <DashboardLayout>
      <div className="page-enter space-y-6 pb-24 lg:pb-8">
        ...
        <div className="space-y-4">
          ...
          {!loading && filtered.map((rx) => {
            const st = STATUS_MAP[rx.status] ?? STATUS_MAP.ACTIVE;
            const refillsLeft = rx.refillsAllowed - rx.refillsUsed;
            return (
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-subtle">
                  <div>
                    ...
                    <p className="text-xs text-muted mt-0.5">
                      Prescribed by {rx.doctor?.name ?? "Unknown doctor"} · {formatDate(rx.issueDate)}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    {rx.status === "ACTIVE" && refillsLeft > 0 && (
                    ...
                    <div key={med.id} className="flex items-start gap-4 px-5 py-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="text-sm font-display font-bold text-primary">{med.medicationName}</p>
