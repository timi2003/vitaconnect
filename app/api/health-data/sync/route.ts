...
function transformRecord(
  ...
  switch (type) {
    ...
    case "Distance": {
      ...
      return { ...base, value: Math.round(meters), unit: "m", recordedAt: toDate(r.endTime) };
    }
    ...
    case "BodyFat": {
      ...
      return { ...base, value: Math.round(pct * 10) / 10, unit: "%", recordedAt: toDate(r.time) };
    }
    case "Nutrition": {
      const kcal = ((r.energy as HCEnergy | undefined)?.inKilocalories) ?? 0;
      return { ...base, value: Math.round(kcal), unit: "kcal", recordedAt: toDate(r.startTime ?? r.endTime) };
    }
    default:
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Normalise: always work with an array of batches
