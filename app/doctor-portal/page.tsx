...
export default function DoctorPortalPage() {
  ...
  // ── Toggle availability → persists to DoctorProfile in DB ─────────────────
  async function handleToggleAvailability() {
    ...
    try {
      ...
      // silently revert — state unchanged
    } finally {
      setToggling(false);
    }
  }

  const greeting = loadingProfile ? "Welcome back" : `Welcome back, Dr. ${doctorName.split(" ")[0]}`;
    ...
    <DoctorDashboardLayout>
      <div className="page-enter space-y-6 pb-24 lg:pb-8">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          {/* Availability toggle — wired to real API */}
          <div className="flex items-center gap-3">
            <span className="text-sm font-display text-secondary">Available for calls</span>
            <label className={cn("relative inline-flex cursor-pointer", toggling && "opacity-60 pointer-events-none")}>
              <input
                type="checkbox"
                checked={isAvailable}
              ...
              <div className={cn(
                ...
                isAvailable
                  ? "bg-teal-500 after:translate-x-5"
                  : "bg-surface-700",
              )} />
            </label>
            {isAvailable && (
            ...
            return (
              <div key={s.label} className={cn("metric-card p-4 border", c.bg, c.border)}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  {s.trend && (
                    <span className="badge badge-success text-xs py-0.5">{s.trend}</span>
                  )}
                </div>
