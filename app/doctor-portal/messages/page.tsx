...
export default function DoctorMessagesPage() {
  ...
  const [pendingRecipient, setPendingRecipient] = useState<PatientResult | null>(null);
  ...
  // ── Search patients to start a new conversation with ───────────────────────
  useEffect(() => {
    if (!search.trim()) {
      setPatientResults([]);
      return;
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        ...
        console.error("[DoctorMessagesPage] patient search failed:", err);
      } finally {
  ...
  function startNewConversation(patient: PatientResult) {
    setPendingRecipient(patient);
    setActiveId(null);
    setMessages([]);
    setSearch("");
    setPatientResults([]);
    ...
    try {
      ...
      const transformed: Conv[] = (raw || []).map((c: any) => {
        ...
        return {
          ...
          unread: false, // last-read comparison can be added once needed
        };
      });
      ...
      console.error("[DoctorMessagesPage] load conversations failed:", err);
    } finally {
  ...
  useEffect(() => {
    ...
    fetch(`/api/messages?conversationId=${activeId}`)
      ...
      .catch((err) => console.error("[DoctorMessagesPage] load messages failed:", err))
  ...
  useEffect(() => {
    ...
    channel.bind("new-message", (incoming: Message) => {
      ...
      loadConversations(); // refresh sidebar preview/order
    });
  ...
  return (
    <DoctorDashboardLayout>
      <div className="page-enter h-[calc(100vh-8rem)] flex gap-0 rounded-2xl overflow-hidden border border-subtle glass">
        ...
        {/* Sidebar */}
        <div className="w-72 flex-shrink-0 border-r border-subtle flex flex-col hidden sm:flex">
          <div className="p-4 border-b border-subtle">
            <h2 className="font-display font-bold text-primary mb-3">Patient Messages</h2>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted" />
              <input className="input pl-9 py-2 text-sm h-9" placeholder="Search patients…"
            ...
            {search.trim() && (
              <div className="mt-2 border border-subtle rounded-xl overflow-hidden">
                ...
                {!searchingPatients && patientResults.map((p) => (
                  <button key={p.id} onClick={() => startNewConversation(p)}
                    ...
                    <p className="text-xs font-display font-semibold text-primary truncate">{p.name}</p>
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-subtle">
            ...
            {filtered.map((c) => (
              <button key={c.id} onClick={() => setActiveId(c.id)}
                ...
                <div className="w-10 h-10 rounded-xl flex items-center justify-center font-display font-bold text-xs flex-shrink-0 bg-brand-600/30 text-brand-300">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {!activeConv && !pendingRecipient ? (
            <div className="flex-1 flex items-center justify-center text-muted text-sm">
              {loadingConvs ? <Loader2 className="w-5 h-5 animate-spin" /> : "Select a conversation or search for a patient"}
            </div>
          ) : (
            <>
              {/* Header */}
              ...
              <div className="px-5 py-4 border-t border-subtle flex-shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <input className="input flex-1 py-2.5 text-sm"
                    placeholder={pendingRecipient ? `Message ${pendingRecipient.name}…` : "Type a message to your patient…"}
                    value={draft}
                  ...
                  <button onClick={handleSend} disabled={!draft.trim() || sending}
                    className={cn(
                      ...
                      draft.trim() && !sending
                        ? "bg-teal-500 text-white hover:bg-teal-400 shadow-glow-sm"
