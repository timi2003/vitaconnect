...
export default function DoctorPatientRecordsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 404 || res.status === 403) { setNotAuthorized(true); return; }
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        console.error("[DoctorPatientRecordsPage] fetch failed:", res.status, body);
        return;
      }
      const { documents } = await res.json();
      setDocs(documents || []);
    } catch (err) {
      console.error("[DoctorPatientRecordsPage] load failed:", err);
    } finally {
      setLoading(false);
    }
  }, [patientId]);
      ...
      <div className="page-enter space-y-6 pb-24 lg:pb-8">
        ...
        {!notAuthorized && (
          <div className="relative">
            ...
            <input className="input pl-10 text-sm" placeholder="Search records, tags…"
          ...
          {loading ? (
            ...
            <div className="glass border border-subtle p-12 text-center">
              ...
              <p className="font-display font-semibold text-secondary">
                {docs.length === 0 ? "This patient hasn't shared any documents yet" : "No documents match your search"}
              </p>
            </div>
          ) : (
            filtered.map((doc) => {
                ...
                <div key={doc.id}
                  ...
                  <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                    <a href={doc.fileUrl} target="_blank" rel="noopener noreferrer"
                      className="p-2 rounded-xl hover:bg-surface-700 text-muted hover:text-primary transition-colors" title="View">
                      <Eye className="w-4 h-4" />
                    </a>
                    <a href={doc.fileUrl} download={doc.title}
                      className="p-2 rounded-xl hover:bg-surface-700 text-muted hover:text-primary transition-colors" title="Download">
